    ...
    <div class="news-detail-page">
      ...
      <div class="article-container" *ngIf="!isLoading && !error && article">
        ...
        <mat-card class="article-card">
          <!-- Article header avec barre colorée -->
          ...
          <!-- Article image en première si présente -->
          <div class="article-media" *ngIf="hasImageFile(article)">
          ...
          <div class="document-section" *ngIf="!hasImageFile(article) && article.fileName">
...
export class NewsDetailComponent implements OnInit {
  ...
  printDocument() {
    if (!this.article) return;
    // Open a new window or tab to print the document
    ...
    // Fetch the document content as blob and create an object URL
    this.newsService.downloadDocument(this.article.id).subscribe({
      next: (blob) => {
        const url = window.URL.createObjectURL(blob);
        ...
        printWindow.focus();
        // Wait a bit and then trigger print
        setTimeout(() => {
          printWindow.print();
          printWindow.close();
          window.URL.revokeObjectURL(url);
